    ...
    def patch(self, request, id):
        ...
        try:
            ...
            serializer = HotelSerializer(
                hotel, data={"availability": not hotel.availability}, partial=True
            )
            if serializer.is_valid():
                serializer.save()
                return Response(
                    {"message": f"Toggled availability for hotel with ID: {id}"}
                )
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        except Hotel.DoesNotExist:
            return Response(
                {"error": "Hotel not found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class HotelUpdateView(RetrieveUpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"success": "Hotel updated successfully"}, status=status.HTTP_200_OK
            )
        except Http404:
            return Response(
                {"error": "Hotel not found"}, status=status.HTTP_404_NOT_FOUND
            ...
            return Response(
                {
                    "errors": {
                        "non_field_errors": [
                            "User is not a superuser or is not authenticated"
                        ]
                    }
                },
                status=status.HTTP_401_UNAUTHORIZED,
...
class RoomTypeDeleteView(DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        instance.delete()
        return Response(
            {"success": "RoomType deleted successfully"},
...
class UpdateNotAvailableDates(APIView):
    ...
    def post(self, request, hotel_id, room_id):
        try:
            ...
            return Response(
                {"message": "Selected dates updated successfully."},
                status=status.HTTP_200_OK,
            )
