...
class AdminUserChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.channel_layer.group_add(f"chat_{self.booking_id}", self.channel_name)
    ...
    async def disconnect(self, close_code):
        await self.channel_layer.group_discard(
            f"chat_{self.booking_id}", self.channel_name
        )
    ...
    async def receive(self, text_data):
        ...
        if sender and booking:
            ...
            await self.send(
                text_data=json.dumps({"error": "Sender or booking not found"})
            )
    ...
    def save_message(self, sender, message_content, timestamp, booking):
        ChatMessage.objects.create(
            message=message_content, sender=sender, timestamp=timestamp, booking=booking
        )
